 ...
 * - Consistent chunk timing (50ms intervals)
 */

// Configuration
const SEND_INTERVAL_MS = 20; // Send audio every 20ms for low latency (reduced from 50ms)
const VOLUME_THRESHOLD_DB = -35; // Below this dB level, audio is considered silence (increased from -50 to filter room noise)
...
function updateConfig(config) {
    if (config.sendIntervalMs !== undefined) {
        // Note: We can't modify const, so we'd need to restructure if this is needed
