"""
HR Interview Service - AI-Powered Real-time HR Interview Platform
...
def generate_system_instruction(job_details: dict, candidate_info: dict) -> str:
    """Generate dynamic system instruction for HR interview based on job and candidate information."""
    ...
    return f"""
You are "Adhira", Senior HR Manager conducting an HR interview for **{job_title}** at **{company_name}**.
...
## INTERVIEW FOCUS
This is an HR/BEHAVIORAL interview - NOT technical!
Focus on:
...
### 2. BEHAVIORAL - STAR METHOD (4-5 questions)
- "Tell me about a time when..."
- "Describe a situation where..."
- "Give an example of..."

### 3. SOFT SKILLS (3-4 questions)
...
### 5. SITUATIONAL (2-3 questions)
- "What would you do if..."
- Hypothetical workplace scenarios

## INTERVIEW STRUCTURE (8-12 minutes, 12-18 questions total)

### Phase 1: WARM INTRO (1-2 min)
- Greet warmly, introduce yourself
- Make candidate comfortable
- This is conversational, not interrogation

### Phase 2: BACKGROUND (2-3 min)
- Career journey
- Current/recent role
- Why looking for change

### Phase 3: BEHAVIORAL (3-4 min)
- Use STAR method questions
- Probe for specific examples
- Ask follow-ups for depth

### Phase 4: SOFT SKILLS (2-3 min)
...
- Thank them warmly
- Direct to End Session button

## PACING
- Keep YOUR responses under 15 seconds
- Be warm and encouraging
- Acknowledge answers: "Thank you for sharing", "That's interesting"
- Don't rush - maintain conversational flow

## CLOSURE (Only after 12+ questions AND 8+ minutes)
Say: "Thank you so much {candidate_name or 'candidate'} for this wonderful conversation. It was great getting to know you. Please click 'End Session' to complete the interview. We'll be in touch with next steps soon. All the best!"

## RULES
- Fresh session - no prior memory
- Be warm and friendly - represent company culture
...
class InterviewSession:
    ...
    def __init__(
        ...
        hr_interview_id: str = None
    ):
        self.session_id = session_id
        self.job_details = job_details
        self.candidate_info = candidate_info
        self.system_instruction = system_instruction
        self.hr_interview_id = hr_interview_id
...
class RegisterSessionRequest(BaseModel):
    ...
    candidate_info: dict
    system_instruction: str
    hr_interview_id: Optional[str] = None
...
class CompleteInterviewRequest(BaseModel):
    """Request to complete an HR interview with AI-generated scores."""
...
async def lifespan(app: FastAPI):
    ...
    print("\n" + "=" * 75)
    print("🎤 HR INTERVIEW SERVICE - Production Hardened")
    print("=" * 75)
...
app = FastAPI(
    title="HR Interview Service",
    description="Production-hardened real-time AI HR interview with jitter buffer and noise filtering",
    version="1.0.0",
    lifespan=lifespan
)
...
class ProductionInterviewSession:
    ...
    def __init__(self, websocket: WebSocket, system_instruction: str, job_details: dict, candidate_info: dict, parent_session: InterviewSession = None):
        ...
        print(f"🆕 Fresh HR interview session initialized for {candidate_info.get('name', 'candidate')}")
    ...
    async def generate_and_store_scores(self):
        ...
        try:
            ...
            # Generate evaluation prompt for HR interview - concise and focused
            evaluation_prompt = f"""
Evaluate this HR/Behavioral interview. Be fair and accurate.

## Context
- Position: {self.job_details.get('title', 'Position')}
- Department: {self.job_details.get('department', '')}
- Candidate: {self.candidate_info.get('name', 'Candidate')}
- Duration: {interview_duration} seconds
- Interview Type: HR/Behavioral Round

## Interview Phases (Expected 12-18 questions total)
1. Introduction (2-3 questions)
2. Behavioral - STAR Method (4-5 questions)
3. Soft Skills (3-4 questions)
4. Cultural Fit (2-3 questions)
...
## Scoring Rules for HR Interview
- Positive acknowledgment = engaged, genuine response
- STAR method answers (Situation, Task, Action, Result) = strong behavioral answers
- Vague responses without examples = weak behavioral answers
- Self-awareness and reflection = positive indicator
- Enthusiasm about role = good cultural fit
- "Let's move on" / silence = weak/skipped answer
...
{{
    ...
    "phase_scores": {{
        ...
        "behavioral": {{"score": <0-100>, "notes": "<brief>"}},
        "soft_skills": {{"score": <0-100>, "notes": "<brief>"}},
        "cultural_fit": {{"score": <0-100>, "notes": "<brief>"}},
        "situational": {{"score": <0-100>, "notes": "<brief>"}}
    }},
    "candidate_strengths": ["<3-5 strengths>"],
    "candidate_weaknesses": ["<1-3 weaknesses>"],
    "candidate_answer_summary": "<2-3 sentences summarizing their answers>",
    "ai_recommendation": "<strongly_recommend|recommend|neutral|not_recommend>",
    "ai_recommendation_reason": "<1-2 sentences>",
    "ai_feedback_summary": "<3-4 sentences overall feedback>",
    "improvement_areas": ["<2-3 areas>"],
    "soft_skills_assessment": [{{"skill_name": "<skill>", "proficiency_level": "<advanced|intermediate|beginner>", "score": <0-100>}}],
    ...
    "interview_completion": {{"phases_covered": <1-5>, "estimated_minutes": <num>, "was_complete": <bool>}},
                                    ...
                                    await session.send(
                                        input=types.LiveClientContent(
                                            turns=[
                                                types.Content(
                                                    role="user",
                                                    parts=[types.Part(text=f"Begin the HR interview NOW. You are Adhira, the HR Manager. Greet {candidate_name} warmly by name, introduce yourself as Adhira - an HR Manager conducting the HR round, mention this is for the {job_title} position. Make them feel comfortable - say something like 'This is just a conversation to get to know you better, no right or wrong answers.' Then start with Phase 1: Introduction by asking them to tell you about themselves. Keep your greeting warm and under 20 seconds.")]
                                                )
                                            ],
                                            turn_complete=True
                                        )
...
async def index(request: Request):
    """Serve the main page."""
    return templates.TemplateResponse("hr_interview.html", {"request": request})
...
async def register_session(request: RegisterSessionRequest):
    ...
    try:
        ...
        active_sessions[session_id] = InterviewSession(
            ...
            hr_interview_id=request.hr_interview_id
        )
...
async def start_interview(candidate_id: str, request: StartInterviewRequest):
    ...
    2. Main service checks:
       ...
       - Candidate passed technical interview
       - HR interview not already taken
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(
                f"{MAIN_SERVICE_URL}/hr-interview/start/{candidate_id}",
                json={
            ...
            hr_interview_id = data.get("hr_interview_id")
            ...
            active_sessions[session_id] = InterviewSession(
                ...
                hr_interview_id=hr_interview_id
            )
            
            print(f"✅ HR Interview session started: {session_id}")
            print(f"   HR Interview ID: {hr_interview_id}")
            ...
            return {
                ...
                "websocket_url": f"ws://localhost:8200/ws/interview/{session_id}",
                "job_details": job_details,
                "candidate_info": candidate_info,
...
async def interview_page(request: Request, session_id: str):
    ...
    session_data = {
        "request": request,
        "session_id": session_id,
        "job_title": "HR Interview",
        "candidate_name": ""
    }
    
    if session_id in active_sessions:
        session = active_sessions[session_id]
        session_data["job_title"] = session.job_details.get("title", "HR Interview")
        session_data["candidate_name"] = f"{session.candidate_info.get('first_name', '')} {session.candidate_info.get('last_name', '')}".strip()
    
    return templates.TemplateResponse("hr_interview.html", session_data)
...
async def websocket_interview(websocket: WebSocket, session_id: str):
    ...
    print(f"🆕 Starting FRESH HR interview session (no prior context)")
...
@app.post("/complete/{hr_interview_id}")
async def complete_interview(hr_interview_id: str, request: CompleteInterviewRequest):
    """
    Complete the HR interview with AI-generated scores.
    ...
    3. Main service stores all scores and updates candidate's hr_interview fields
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(
                f"{MAIN_SERVICE_URL}/hr-interview/complete/{hr_interview_id}",
            ...
            print(f"✅ HR Interview completed: {hr_interview_id}")
