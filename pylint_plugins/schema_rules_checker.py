...
class SchemaRulesChecker(BaseChecker):
    ...
    def _get_field_assignments(self, node):
        return [
            n for n in node.body if isinstance(n, (astroid.Assign, astroid.AnnAssign))
        ]
    ...
    def _check_schema_name(self, node):
        ...
        if not node.name.endswith("Schema"):
            self.add_message(
                "schema-name-must-end-with-schema", node=node, args=(node.name,)
            )
        ...
        for assign in self._get_field_assignments(node):
            if isinstance(assign, astroid.Assign):
                ...
                field_value = (
                    ...
                    if hasattr(assign.value, "as_string")
                    else ""
                )
                field_annotation = ""
                ...
                field_annotation = (
                    assign.annotation.as_string() if assign.annotation else ""
                )
                field_value = assign.value.as_string() if assign.value else None
            else:
                continue

            if self._check_boolean_field(
                assign, field_name, field_annotation, field_value
            ):
                continue
    ...
    def _check_boolean_field(self, assign, field_name, field_annotation, field_value):
        ...
        if field_annotation == "bool":
            if not field_name.startswith("is_"):
                ...
                field_value
                and "default=True" not in field_value
                and field_value != "True"
            ):
                self.add_message("boolean-field-rule", node=assign, args=(field_name,))
    ...
    def _check_field_validation(
        self, assign, field_name, field_annotation, field_value
    ):
        ...
        if field_name not in ["model_config", "ConfigDict"]:
            if field_value and "Field(" not in field_value:
                self.add_message(
                    "field-validation-rule", node=assign, args=(field_name,)
                )
            elif (
                isinstance(assign, astroid.AnnAssign)
                and not field_value
                and not field_annotation.startswith("Optional[")
            ):
                if field_annotation in [
                    "str",
                    "int",
                    "bool",
                    "float",
