...
def execute_request(
    base_url: str, method: str, endpoint: str, data: str, headers: dict
):
        ...
        for op in sorted_operations:
            ...
            if path_params_in_current_path:
                ...
                if all(param in created_ids for param in path_params_in_current_path):
                    try:
                        ...
                        print(
                            f"Skipping endpoint {path} due to missing path parameter: {e}"
                        )
                        continue
                else:
                    print(
                        f"Skipping endpoint {path} due to missing path parameters: {set(path_params_in_current_path) - set(created_ids.keys())}"
                    )
                    continue
            ...
            if "requestBody" in operation:
                ...
                if "application/json" in content:
                    ...
                    example_to_use = None
                    if (
                        "examples" in app_json_content
                        and "default" in app_json_content["examples"]
                        and "value" in app_json_content["examples"]["default"]
                    ):
                        example_to_use = app_json_content["examples"]["default"][
                            "value"
                        ]
                    ...
                    if example_to_use:
                        ...
                        if ref:
                            ...
                            if schema_component:
                                all_schemas = schema.get("components", {}).get(
                                    "schemas", {}
                                )
                                simplified_obj = simplify_schema(
                                    schema_component, all_schemas
                                )
                                request_body_for_request = json.dumps(simplified_obj)
                                request_body_schema_simplified = (
                                    request_body_for_request
                                )
                        ...
                        if isinstance(obj, dict):
                            for key, value in obj.items():
                                if key in id_map:
                                    obj[key] = id_map[key]
                                    print(
                                        f"Substituted '{key}' with '{id_map[key]}' in request body for {path}"
                                    )
            ...
            status_code, response_text = execute_request(
                base_url,
                method_upper,
                formatted_path,
                request_body_for_request,
                    ...
                    def find_and_store_ids(data, param_names, storage):
                        if isinstance(data, dict):
                            for key, value in data.items():
                                if key in param_names and isinstance(
                                    value, (str, int, float, bool)
                                ):
                                    if key not in storage:
                                        storage[key] = value
                                        print(
                                            f"Captured {{'{key}': '{value}'}} from successful POST to {path}"
                                        )
                                elif isinstance(value, (dict, list)):
