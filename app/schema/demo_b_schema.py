...
class DemoBCreateSchema(BaseAppSchema):
    ...
    description: Optional[str] = Field(default=None, max_length=500, example="This is a sample demo B description.")
    is_active: Optional[bool] = Field(default=True, example=True)
    status: DemoBStatus = Field(..., description="Current status of the entity", example=DemoBStatus.CREATED)


class DemoBUpdateSchema(BaseAppSchema):
    """Schema for updating an existing demo B."""
    name: Optional[str] = Field(default=None, min_length=1, max_length=200, example="Updated Demo B Name")
    ...
    status: Optional[DemoBStatus] = Field(default=None, description="Current status of the entity", example=DemoBStatus.UPDATING)
...
class DemoBIsActiveUpdateSchema(BaseAppSchema):
    ...
    is_active: bool = Field(default=True, description="Whether the demo B is active", example=False)
...
class DemoBReadSchema(BaseAppSchema):
    ...
    is_active: Optional[bool] = Field(default=True, example=True)
    status: DemoBStatus = Field(..., description="Current status of the entity", example=DemoBStatus.CREATED)
    demo_b_id: UUID = Field(..., description="Demo B ID", example="a3d8f6b0-3c1b-4e6f-8a2d-9c8e7f6a5b4d")
