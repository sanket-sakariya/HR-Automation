...
class DemoBService(BaseAppService):
    ...
    async def create(
        self, payload: DemoBCreateSchema, user_id: UUID, workspace_id: UUID
    ) -> DemoBReadSchema:
        """Create a new demo."""
        ...
        demo_b = await self.demo_repo.insert(
            demo_b_data=demo_b_data, user_id=user_id, workspace_id=workspace_id
        )
        log_user_activity(
            f"{LogMessages.DEMO_CREATED}: {demo_b.name}",
    ...
    async def read(
        ...
        """Read a demo."""
        demo_b = await self.demo_repo.get_by_id(
    ...
    async def list_all(
        ...
        if isinstance(result, dict):
            ...
            schema_data = [
                DemoBReadSchema.model_validate(demo_b_item) for demo_b_item in data
            ]
            return {"data": schema_data, "pagination": pagination}

        # Fallback for non-dict results
        schema_data = [
            DemoBReadSchema.model_validate(demo_b_item) for demo_b_item in result
        ]
    ...
    async def update(
        ...
        """Update a demo."""
        existing_demo_b = await self.demo_repo.get_by_id(
            demo_b_id=demo_b_id, workspace_id=workspace_id
        )
        ...
        demo_b = await self.demo_repo.update(
            demo_b_id=demo_b_id,
            demo_b_data=payload_dict,
            user_id=user_id,
            workspace_id=workspace_id,
        )

        log_user_activity(
            f"{LogMessages.DEMO_UPDATED}: {demo_b.name}", action_type="demo_update"
        )
        return DemoBReadSchema.model_validate(demo_b)
    ...
    async def delete(self, demo_b_id: UUID, user_id: UUID, workspace_id: UUID) -> None:
        ...
        log_user_activity(
            f"{LogMessages.DEMO_DELETED} {demo_b_id}", action_type="demo_delete"
        )

    async def update_status(
        self,
        demo_b_id: UUID,
        payload: DemoBStatusUpdateSchema,
        user_id: UUID,
        workspace_id: UUID,
    ) -> DemoBReadSchema:
        """
        ...
        """
        log_central(
            message=f"{LogMessages.DEMO_STATUS_UPDATED} {demo_b_id} status to {payload.status}",
            level="info",
        )
