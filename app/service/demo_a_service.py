...
class DemoAService(BaseAppService):
    ...
    async def read(
        ...
        """Read a demo."""
        demo_a = await self.demo_repo.get_by_id(
    ...
    async def list_all(
        ...
        if isinstance(result, dict):
            ...
            schema_data = [
                DemoAReadSchema.model_validate(demo_a_item) for demo_a_item in data
            ]
            return {"data": schema_data, "pagination": pagination}

        # Fallback for non-dict results
        schema_data = [
            DemoAReadSchema.model_validate(demo_a_item) for demo_a_item in result
        ]
    ...
    async def update(
        ...
        log_user_activity(
            f"{LogMessages.DEMO_UPDATED}: {demo_a.name}", action_type="demo_update"
        )
        return DemoAReadSchema.model_validate(demo_a)
    ...
    async def delete(self, demo_a_id: UUID, user_id: UUID, workspace_id: UUID) -> None:
        ...
        log_user_activity(
            f"{LogMessages.DEMO_DELETED} {demo_a_id}", action_type="demo_delete"
        )
